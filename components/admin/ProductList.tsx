...
export default function ProductList({
  ...
  const filterLabels: Record<AdminProductFilter, string> = {
    all: t.admin.filterAll,
    published: t.admin.filterPublished,
    hidden: t.admin.filterHidden,
    "out-of-stock": t.admin.filterOutOfStock,
    "on-sale": t.admin.filterOnSale,
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`"${name}" — ${t.admin.deleteProductConfirm}`)) return;
    setDeleting(id);
      ...
      {filter !== "all" && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand/20 bg-brand-muted/40 px-4 py-3">
          <p className="text-sm font-medium text-slate-700">
            {t.admin.filterLabel}{" "}
            <span className="text-brand">{filterLabels[filter]}</span>
            <span className="ml-2 text-slate-500">
              ({filteredProducts.length} {t.admin.filterCount})
            </span>
          </p>
          <Link
            href="/admin/products"
          ...
          <table className="w-full min-w-[800px] text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                ...
                <th className="px-4 py-3">{t.admin.colSku}</th>
                <th className="px-4 py-3">{t.admin.colPrice}</th>
                <th className="px-4 py-3">{t.admin.colStock}</th>
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    >
                      {product.inStock
                        ? product.stockQuantity ?? t.admin.stockYes
                        : t.admin.stockNo}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(product.id, product.nameSq || product.name)
                        }
